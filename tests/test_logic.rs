...
fn test_ac_csdp_actor_logic_learning() {
    ...
    let mut model = RLModel3::new(2, 2, vec![64, 32], vec![32], &device, dt, None)
    ...
    for epoch in 1..=50 {
        ...
        for (input_vec, class_idx) in &pairs {
            ...
            for _ in 0..n_timesteps {
                noise_sequence.push(Tensor::randn(0.0f32, 0.5f32, (2, 1), &device).unwrap());
            }
            ...
            for _pass in 0..2 {
                ...
                let label = if _pass == 0 { 1.0 } else { 0.0 };
