...
pub struct AlgorithmFFMulti1 {
    pub model: FFMultiModel,
...
impl Algorithm for AlgorithmFFMulti1 {
    fn run(
        &mut self,
        ...
        for episode in 1..=self.n_episodes {
            ...
            for _step in 0..self.n_steps_per_episode {
                ...
                let goodness_scores = self.model.predict_scores(&current_states_tensors)?;
                let scores_vec: Vec<f32> = goodness_scores.flatten_all()?.to_vec1()?;
