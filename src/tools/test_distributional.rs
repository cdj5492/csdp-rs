...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    loop {
        terminal.draw(|f| {
            ...
            let csdp_data: Vec<(String, u64)> = csdp_probs.iter().enumerate().map(|(i, &p)| (format!("{:02}", i), f32::min(p * 100.0, 100.0) as u64)).collect();
            ...
            let ff_data: Vec<(String, u64)> = ff_probs.iter().enumerate().map(|(i, &p)| (format!("{:02}", i), f32::min(p * 100.0, 100.0) as u64)).collect();
        ...
        if last_tick.elapsed() >= tick_rate {
            ...
            let v_eval = current_target / 100.0;
            let norm_eval = (v_eval.powi(2) + 1.0).sqrt();
            let x_eval = Tensor::from_vec(vec![v_eval / norm_eval, 1.0 / norm_eval], (1, input_size), &device)?;
            ...
            if let Ok(ff_scores) = ff.predict_scores(&[x_eval.clone()]) {
                if let Ok(ff_flat) = ff_scores.flatten_all().and_then(|t| t.to_vec1::<f32>()) {
                    ...
                    let exps: Vec<f32> = ff_flat.iter().map(|&v| ((v - max_v) / tau_ff).exp()).collect();
            ...
            if let Ok(csdp_scores) = csdp.predict_scores(&[x_eval.clone()]) {
                if let Ok(csdp_flat) = csdp_scores.flatten_all().and_then(|t| t.to_vec1::<f32>()) {
                    ...
                    let exps: Vec<f32> = csdp_flat.iter().map(|&v| ((v - max_v) / tau_csdp).exp()).collect();
                    let sum: f32 = exps.iter().sum();
